...
describe('ShippingManagement', () => {
  ...
  it('deletes a shipping option after confirmation', async () => {
    const sampleOptions = [
      {
        id: 1,
        name: 'Standard Shipping',
    ...
    ;(ShippingService.getAllShippingOptions as any).mockResolvedValue({ data: sampleOptions })
    ;(ShippingService.deleteShippingOption as any).mockResolvedValue({})
    ...
    // Click the delete button for the option
    const deleteButton = wrapper.find('button.btn-icon.delete')
    await deleteButton.trigger('click')
    await flushPromises()

    expect(confirmSpy).toHaveBeenCalled()
    expect(ShippingService.deleteShippingOption).toHaveBeenCalledWith(1)
  })
  ...
  it('does not delete shipping option if confirmation is cancelled', async () => {
    ...
    expect(confirmSpy).toHaveBeenCalled()
    expect(ShippingService.deleteShippingOption).not.toHaveBeenCalled()
  })
  ...
  it('displays error when deleting shipping option fails', async () => {
    ...
    expect(confirmSpy).toHaveBeenCalled()
    expect(wrapper.find('.error-message').exists()).toBe(true)
    expect(wrapper.text()).toContain('Failed to delete shipping option')
  })

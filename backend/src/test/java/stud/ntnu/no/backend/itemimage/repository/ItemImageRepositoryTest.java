...
public class ItemImageRepositoryTest {
    ...
    public void testFindByItemId_WhenItemHasImages_ShouldReturnImages() {
        ...
        assertThat(images).extracting(ItemImage::getImageUrl)
    ...
    public void testFindByItemIdOrderByDisplayOrder_ShouldReturnImagesInOrder() {
        ...
        Item item = TestHelper.createBasicItem(entityManager, "Item with ordered images", seller, category);
        ...
        // Act
        List<ItemImage> orderedImages = itemImageRepository.findByItemIdOrderByDisplayOrder(item.getId());

        // Assert
        assertThat(orderedImages).hasSize(3);
        assertThat(orderedImages).extracting(ItemImage::getImageUrl)
                                .containsExactly(

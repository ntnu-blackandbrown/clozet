...
public class HistoryServiceImplTest {
    ...
    void getUserHistory_shouldReturnActiveUserHistory() {
        ...
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        List<History> historyList = Arrays.asList(history);
        when(historyRepository.findByUserAndActiveOrderByViewedAtDesc(user, true)).thenReturn(historyList);
    ...
    void getUserHistory_withInvalidId_shouldThrowException() {
        ...
        verify(userRepository, times(1)).findById(99L);
        verify(historyRepository, never()).findByUserAndActiveOrderByViewedAtDesc(any(User.class), anyBoolean());
    }
} 
...
public class ImageServiceImplTest {
  ...
  void setUp() {
    ...
    validImageFile =
        new MockMultipartFile(
            "image.jpg", "image.jpg", "image/jpeg", "test image content".getBytes());

    emptyFile = new MockMultipartFile("empty.jpg", "empty.jpg", "image/jpeg", new byte[0]);

    invalidTypeFile =
        new MockMultipartFile(
            "document.txt", "document.txt", "text/plain", "text content".getBytes());
  }
  ...
  void uploadImage_withInvalidFileType_shouldThrowException() {
    ...
    // Act & Assert
    assertThrows(
        InvalidFileTypeException.class, () -> imageService.uploadImage(invalidTypeFile, 1L));
  ...
  void uploadImage_withItemNotFound_shouldThrowException() throws IOException {
    ...
    when(fileStorageService.storeFile(any(MultipartFile.class), eq(99L)))
        .thenReturn("/images/99/test.jpg");

...
class FavoriteControllerTest {
            ...
            document(
                ...
                responseFields(
                    ...
                    fieldWithPath("[].createdAt")
                        .description("Date and time when favorite was created"),
                    fieldWithPath("[].updatedAt")
                        .description("Date and time when favorite was last updated"))));
  ...
  void getFavoritesByUserId_ShouldReturnUserFavorites() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(get("/api/favorites/user/user1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].id").value(1))
        .andExpect(jsonPath("$[0].userId").value("user1"))
  ...
  void updateFavorite_ShouldReturnUpdatedFavorite() throws Exception {
    ...
    when(favoriteService.updateFavorite(eq(1L), any(CreateFavoriteRequest.class)))
        .thenReturn(updatedFavorite);
  ...
  void deleteFavorite_ShouldReturnNoContent() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(delete("/api/favorites/1"))

...
class CloudinaryStorageServiceTest {
    ...
    void storeFile_withDifferentItemIds_shouldUseCorrectFolders() throws IOException {
        ...
        when(uploader.upload(any(byte[].class), argThat(params -> "items/1".equals(params.get("folder")))))
            .thenReturn(uploadResult1);
        when(uploader.upload(any(byte[].class), argThat(params -> "items/2".equals(params.get("folder")))))
        ...
        verify(uploader, times(1)).upload(any(byte[].class), argThat(params -> 
            "items/1".equals(params.get("folder"))));
        verify(uploader, times(1)).upload(any(byte[].class), argThat(params -> 
            "items/2".equals(params.get("folder"))));
    }

...
class ShippingOptionControllerTest {
  ...
  @InjectMocks private ShippingOptionController shippingOptionController;

  @BeforeEach
  void setUp(RestDocumentationContextProvider restDocumentation) {
    mockMvc =
        MockMvcBuilders.standaloneSetup(shippingOptionController)
            .apply(
  ...
  void getAllShippingOptions_ShouldReturnListOfShippingOptions() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(get("/api/shipping-options"))
  ...
  void getShippingOption_ShouldReturnShippingOptionById() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(get("/api/shipping-options/" + optionId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
        .andExpect(jsonPath("$.name").value("Standard Shipping"))
        .andDo(
            document(
                "shipping-option-get-by-id",
                Preprocessors.preprocessRequest(prettyPrint()),
                Preprocessors.preprocessResponse(prettyPrint()),
                responseFields(
                    fieldWithPath("id").description("Shipping option ID"),
  ...
  void createShippingOption_ShouldReturnCreatedShippingOption() throws Exception {
    ...
    when(shippingOptionService.createShippingOption(any(CreateShippingOptionDTO.class)))
  ...
  void updateShippingOption_ShouldReturnUpdatedShippingOption() throws Exception {
    ...
    when(shippingOptionService.updateShippingOption(
            eq(optionId), any(CreateShippingOptionDTO.class)))
        .thenReturn(updatedOption);

    // When/Then
    mockMvc
        .perform(
            put("/api/shipping-options/" + optionId)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    verify(shippingOptionService)
        .updateShippingOption(eq(optionId), any(CreateShippingOptionDTO.class));
  }
  ...
  void deleteShippingOption_ShouldReturnNoContent() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(delete("/api/shipping-options/" + optionId))
        .andExpect(status().isNoContent())
        .andDo(
            document(
                "shipping-option-delete",
                Preprocessors.preprocessRequest(prettyPrint()),
                Preprocessors.preprocessResponse(prettyPrint())));

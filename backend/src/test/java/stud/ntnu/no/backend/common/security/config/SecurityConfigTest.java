...
class SecurityConfigTest {
  ...
  void corsConfigurationSource_ShouldConfigureCorsCorrectly() {
    ...
    boolean hasOrigin =
        configForLocal.getAllowedOrigins() != null
            && configForLocal.getAllowedOrigins().contains(localOrigin);
    boolean hasOriginPattern =
        configForLocal.getAllowedOriginPatterns() != null
            && !configForLocal.getAllowedOriginPatterns().isEmpty();

    assertTrue(
        hasOrigin || hasOriginPattern,
        "CORS configuration should have either allowed origins or origin patterns");
    assertTrue(
        configForLocal
            .getAllowedMethods()
            .containsAll(
    ...
    hasOrigin =
        configForProd.getAllowedOrigins() != null
            && configForProd.getAllowedOrigins().contains(prodOrigin);
    hasOriginPattern =
        configForProd.getAllowedOriginPatterns() != null
    ...
    assertTrue(
        configForUnknown.getAllowedOrigins() == null
  ...
  void normalizeOrigin_ShouldHandleEdgeCases() throws Exception {
    ...
    assertEquals(
        "http://example.com", normalizeOriginMethod.invoke(securityConfig, "http://example.com/"));

    // Test with whitespace
    assertEquals(
        "http://example.com",
        normalizeOriginMethod.invoke(securityConfig, "  http://example.com  "));

...
/**
 * Helper class for providing test entities for ItemImage repository tests.
 */
public class TestHelper {
    ...
    /**
     * Creates a basic item image with required fields.
     * 
     * @param entityManager test entity manager
     * @param item the item to associate the image with
     * @param url the URL of the image
     * @param displayOrder the display order of the image
     * @return the created and persisted item image
     */
    public static ItemImage createItemImage(TestEntityManager entityManager, Item item, String url, int displayOrder) {
        ItemImage itemImage = new ItemImage();
        itemImage.setItem(item);
        itemImage.setImageUrl(url);
        itemImage.setDisplayOrder(displayOrder);
        itemImage.setPrimary(displayOrder == 0); // Set as primary if it's the first image
